...
export default class AuthController {
  public async register({ request, response }: HttpContext) {
    const userData = request.only(['nick', 'email', 'password', 'firstName', 'lastName', 'dateOfBirth', 'gender', 'phoneNumber'] as any)

    userData.mode = 'system';
    userData.notifications = 'sound';
    userData.activeStatus = 'active';

    const requiredFields = ['nick', 'email', 'password', 'firstName', 'lastName', 'dateOfBirth', 'gender', 'phoneNumber'];
    ...
    if (!allowedGenders.includes(userData.gender)) {
      return response.badRequest({ error: 'Invalid gender. Allowed values: male, female, nonbinary, other' });
    }
